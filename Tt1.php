...
class Tt2 extends CI_Controller { 
	...
	function _ajax()
	{
		...
		$this->xajax->configure("requestURI", base_url().'index.php/tt2/');	# index.php/controller/
	...
	function getProducts($style=0) {
		$data['all_classes'] = $this->tt_model->get_all_classes($style,FALSE);
...
//		  $objResponse->assign("update".$count,"disabled",false);
	...
	$region = $this->tt_model->get_region_name($region_id); 
...
//echo "selected region= " . $this->session->userdata('region_id');	
		...
		$data['records'] = $this->tt_model->get_all_classes(0,TRUE);
	...
	//	$data['region'] = $this->tt_model->get_region_name($region_id); 
		$data['regions'] = 0;//$this->tt_model->get_Regions(); 
		$data['region_id'] = $region_id; 
		$data['region_name'] =  $this->tt_model->get_region_name($region_id);;
	...
	function tt_paginate_featured()
	{
		...
		$data['records'] = $this->tt_model->get_paginated_classes_featured($config['per_page'], $this->uri->segment(3));
	...
	{	
		...
		$config['total_rows'] = $this->tt_model->get_rows_featured(); // only number of rows
...
function _head($title="",$data=array())
{
		...
		$this->load->view('tt2/blocks/head',$data); 
}
...
function region_selected()
	...
	{	
		...
		$data['records'] = $this->tt_model->get_Regions(); 
...
// classes
	...
	{	
		$config['base_url'] = site_url().'/tt2/tt_paginate/';
		$config['per_page'] = 99;
		$config['num_links'] = 3;
		$config['full_tag_open'] = '<div id="pagination">';
		$config['full_tag_close'] = '</div>';
		$config['total_rows'] = $this->tt_model->get_rows(); // only number of rows
		$config['first_link'] = 'First';
		$config['last_link'] = 'Last';
		$config['next_link'] = 'Next';
		$config['prev_link'] = 'Prev';
	...
	function classes()
	{	
		$this->tt_model->get_all_classes(0,0); // to initialize the variables
		$config = $this->init_pagination();
		$this->pagination->initialize($config);
		$data['row_numbers'] = $config['total_rows'];
		$data['records'] = $this->tt_model->get_all_classes($config['per_page'],0);
		$data['region'] = $this->tt_model->get_region_name($this->session->userdata('region_id')); 
		$this->view_classes($data);
	}
	function tt_paginate()
	{
		...
		$this->session->set_userdata('p_offset', $this->uri->segment(3));  // save it for later
		$data['region'] = $this->tt_model->get_region_name($this->session->userdata('region_id')); 
		$data['records'] = $this->tt_model->get_paginated_classes($config['per_page'], $this->uri->segment(3));
		$this->view_classes($data);
	}
	function view_classes($data)
	{		
		$this->_head("Classes - Treks and Tracks");
		$this->load->view('tt2/collection',$data);
	}
...
(0,$activity_id);
		...
		$data['activities_related'] = $this->tt_model->get_related_activities($activity_id);
		$data['records'] = $this->tt_model->get_class($activity_id);
		...
		$data['region_name'] = $this->load->view('tt2/blocks/top_bar',$data,true);
		...
		$this->load->view('tt2/blocks/head',$head_data); 
		$this->load->view('tt2/product',$data);
	}
...
//print_r($data['records']);     
		...
		$this->load->view('tt2/blocks/head',$head_data); 
		$this->load->view('tt2/booking1',$data);
	}
...
//	echo "update1=".$this->input->post('update1');
	...
	//die();
			...
			$this->load->view('tt2/blocks/head',$head_data); 
			$this->load->view('tt2/booking2',$data);
	...
	function show_summary($event_group_id,$ledger_id,$err_num = 0)
	{
		...
		$this->load->view('tt2/blocks/head',$head_data); 
		$this->load->view('tt2/summary',$data);
	...
	$redirect_url = site_url()."tt2/class_detail/9"; // Where the user will end up.
	$arb = false;
	$valid = false;
...
//print_r($fields); die();
			...
			if($fields['x_response_code'] == 1)
			{
				...
				$redirect_url = site_url()."tt2/cc_success/".$event_group_id; // Where the user will end up.
			}
			else	
			{
				if (! $event_group_id) $event_group_id = 0;  //placeholder
//				if (! $this->input->post('x_first_name') && ! $this->input->post('x_last_name'))
//					$redirect_url = site_url()."tt2/product/". $activity_id  . "/" .$response_reason_code; // 
//				else
					$redirect_url = site_url()."tt2/show_summary/". $event_group_id . "/" . $ledger_id . "/" .$response_reason_code; // 
			}
...
//print_r($data['records']); die();
		...
		$this->load->view('tt2/blocks/head',$head_data); 
		$this->load->view('tt2/discount_success',$data);
}
...
//print_r($data['records']); die();
		...
		$this->load->view('tt2/blocks/head',$head_data); 
		$this->load->view('tt2/cc_success',$data);
