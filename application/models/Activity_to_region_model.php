...
class Activity_to_region_model extends CI_Model {
	...
	function get_region_records($activity_id)
	{
		$this->db->where('activity_id',$activity_id);
		$query = $this->db->get('region');
		return $query->result();
	}
	function get_region_string($activity_id)
	{
		$regions_string = '';
		$this->db->select('*');
		$this->db->from('activity_to_region');
	...
	function add_record($activity_id,$region_count) 
	{
...
//echo "region_id_val =	" .	$region_id_val . "<br>";
				...
				$this->db->query("REPLACE INTO activity_to_region (activity_id, region_id) VALUES ('$activity_id','$region_id_val')");
				};
			};
		return;
	}
	...
	function delete_record($activity_id,$region_id)
	{   
		$this->db->where('activity_id', $activity_id);
