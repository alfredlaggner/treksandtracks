...
class Ledger_model extends CI_Model {
	...
	{
//if ($to_excel) {		
//echo $to_excel.$ledger_id.$activity_id.$location_id.$region_id.$event_date_from.$event_date_to.$booking_date_from.$booking_date_to.$customer_id;
//
//die();}
		...
		if($ledger_id) $this->db->where('ledger.ledger_id', $ledger_id);
		if($activity_id) $this->db->where('ledger.activity_id', $activity_id);
		if($location_id) $this->db->where('ledger.location_id', $location_id);
		if($customer_id) $this->db->where('ledger.customer_id', $customer_id);
		...
		$this->db->limit(500);
//		if ($ledger_id or $activity_id or $location_id or $customer_id or $region_id) $this->db->limit(999999);
...
//						echo $row['email'];
//						echo  "<br>";
					//	$output[]= array($row['first_name'].','.$row['last_name'].','.$row['email']);
						$output[]= array($row->first_name,$row->last_name,$row->email);
					}					
//print_r($output);						
//die();					
					$this->excel->getActiveSheet()->fromArray($output, NULL, 'A2');
					$filename='Generated_mailing_list.xls'; //save our workbook as this file name
...
"WHERE  ledger.event_id = $event_id "
 ;
	...
	function xcalc_fixed_discount($event_group_id) // calculate fixed discount
	{
		...
		if ($rest_discount <=0) $rest_discount =0;  // avoid negative values
			$data = array(
