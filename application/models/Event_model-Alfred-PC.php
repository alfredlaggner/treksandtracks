...
//		->where('event.activity_id', $activity_id)
	...
	function get_by_location_v1($activity_id)
	{
		...
		$this->db->join('location','event.location_id=location.location_id');
		$this->db->where("event.is_deleted",FALSE);
//		->join('activity', 'activity.activity_id = event.activity_id')
//		->join('rate_price', 'rate_price.activity_id = activity.activity_id', 'left')
	...
	function get_record($event_id)
	{
		...
		->select('location.name as location_name,location.directions as location_directions, activity.name as activity_name, activity.code as activity_code, activity.*, event.*, location.* ')
