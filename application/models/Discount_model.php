...
//					{$data['messages'] = "Could not copy CSV file to temporary directory ready for importing."; 
//					die();}
//$server_dir = "/wamp/www/tt/groupon_codes.csv";
//$server_dir = '/home/users/web/b1458/moo.terramarinfo/treksandtracks/groupon_codes.csv';
//$server_dir = LOCAL_SERVER_DIR.'groupon_codes.csv';
//$server_dir = TEST_SERVER_DIR.'groupon_codes.csv';
$server_dir = PROD_SERVER_DIR.'groupon_codes.csv';
$err_code=0;
		$data = array();
//echo "target = ". $server_dir;	die();
//$prefs = array(
//                'tables'      => array('discount', 'discount_to_activity'),  // Array of tables to backup.
...
//	die("error- files not backed up");
//else log_message('error', 'Backup successful.');
//
//if (! write_file('./discount_backup.zip', $backup)) 
//	die("error- files not written");
//else log_message('error', 'Backup written.');
//	
//if (!force_download('discount_backup.zip', $backup))
//	die("error- files not downloaded");
//else log_message('error', 'Backup downloaded.');
//die();	
...
//echo $server_dir;	
		...
		$query = $this->db->query(
		"LOAD DATA INFILE ? REPLACE INTO TABLE discount_work 
		FIELDS TERMINATED BY ',' OPTIONALLY ENCLOSED BY '\"' LINES TERMINATED BY '\\n' 
		...
		if ($query) {
			...
			$data['messages'] = array('name' => "Import failed.");
die("Import into discount_work failed!");			
		}
...
//print_r($row);
		...
		if (! isset($discount))	{die("<br>Nothing imported! Did you try to import same codes again?");};
...
//		$data1 = array('activity_id' => $activity);
//		$this->db->where('activity_id', 0);			// assign activity phase 2
//		$query = $this->db->update('discount_to_activity_work', $data1);
//		if ($query) {
//			 $data['messages'] = array('name' => "All items added successfully to discount_to_activity_work (Phase II) .");
//		} else {
//			$data['messages'] =  array('name' => "Add to discount_to_activity_work failed. (Phase II)");
//		}
//		echo $data['messages'];
