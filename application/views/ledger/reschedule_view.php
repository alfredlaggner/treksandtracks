...
<input type="hidden"  name="event_group_id" id="event_group_id"   value='<?= $row->event_group_id ?>'  />
<input type="hidden"  name="payment_id" id="payment_id"   value='<?= $row->payment_id ?>'  />
<input type="hidden"  name="main_customer" id="main_customer"   value='<?= $row->main_customer ?>'  />
...
<input type="hidden"  name="discount_amount_type" id="discount_amount_type"   value='<?= $row->discount_amount_type ?>'  />
...
<input type="hidden"  name="cc_amount" id="cc_amount"   value='<?= $row->cc_amount ?>'  />
					...
					<li>
						...
						<select   onchange="xajax_getActivities(<?=$row->activity_id ?>,this.value );return false;"  
						type="text" name="location_code" id="location_code"  class="text"  value='<?=  $row->location_code ?>'  />
						<?

						if(isset($locations)) : foreach($locations as $location) : ?>
						<? if ($row->location_id == $location->location_id) : ?>
						<option selected value="<?=  $location->location_id; ?>"><?=  $location->name ?></option>
						<? else : ?>
...
<div id="activity_display">						
						<select type="text"  name="event_id" id="event_id"  class="text" value=" <?= $row->event_id ?> />
						<? if(isset($events)) : foreach($events as $event) : ?>
						<? if ($event['event_event_id'] == $row->event_id) : ?>
						<option selected value="<?=  $event['event_event_id'] ?>"><?= date('D, M j',strtotime($event['event_date'])) .' ' . date('g:i',strtotime($event['event_time'])). ' '.  $event['attending']. '/'. $event['capacity']?></option>
						<? else : ?>
