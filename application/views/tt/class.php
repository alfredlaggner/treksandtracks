...
-->
	...
	<div class="blog">
		...
		<? $i = 1; ?>
		<? foreach ($events as $event) : ?>
		<div style=" padding: 0.4em; padding-left: 1.5em;  padding-right: 1.5em; display: none; " id="do_book<?= $j ?><?= $i ?>" title="Number of Students and Promo Code">
...
echo form_open('tt/class_booking/'. $event['event_event_id'] . "/0" ,$attributes);?>
				...
				<fieldset>
					<div>
						...
						<input type="hidden" name="event_date" id="event_date<?= $j ?><?= $i ?>" value="<?= $event['event_date']; ?>" />
...
"xajax_verify_promo_code(xajax.getFormValues('booking<?= $j ?><?= $i ?>'),<?= $j ?><?= $i ?>);return false;" size="10" id="promo_code<?= $j ?><?= $i ?>"  name="promo_code" value="">
					</div>
				</fieldset>
				<div>
					<input type="submit"  id="update<?= $j ?><?= $i ?>"  name="update<?= $j ?><?= $i ?>" value="Continue" class="submit" />
				</div>
				</form>
				</p>
