...
<div class="page-title-container">
		<div class="container">
				<div class="page-title pull-left">
						<h2 class="entry-title">Checkout for <?= $row->name ?></h2>
				</div>
...
<article>
		...
		<p class="title">Don't see the date or location that works for you? Email us at <a href="mailto:info@treksandtracks.com">info@treksandtracks.com</a></p>
		<? else : ?>
		<h3>Choose Location and Time</h3>
		<? endif ?>
		<? $j=1; ?>
				...
				<div id="wrap_location<?= $j; ?>">
					...
					Price
						...
						<div>
								...
								<a  title="Click to select date and time" style="font-size:12px; text-decoration:none" href="#" id="go_book<?= $disable ?><?= $j ?><?= $i ?>"  > 
...
<? $i = 1; ?>
<? foreach ($events as $event) : ?>
<div style=" padding: 0.4em; padding-left: 1.5em;  padding-right: 1.5em; display: none; " id="do_book<?= $j ?><?= $i ?>" title="You chose <?= date('D, M j',strtotime($event['event_date'])) ?>   (Press ESC to go back)">
...
echo form_open('tt_v2/class_booking/'. $event['event_event_id'] . "/0" ,$attributes);?>
				...
				<input type="hidden"  name="exp_discount_price" id="exp_discount_price<?= $j ?><?= $i ?>" value="<?= $event['exp_discount_price'] ?>" />
				<ul>
						<li>
								<label for="participants">How many are you? <a class="tooltip" title="If you have NO discount code enter number of students. <br>If you have a discount code please enter one at a time. <br><br>Question? Call us at 650-557-4893 Mon-Fri, 9-5 PST or email info@treksandtracks.com" href="#"> Help</a></label>
						</li>
						<li>
						...
						<br>
						<li>
								<label for="promo_code"   >Enter Discount Code (Crtl-V to paste) <a class="tooltip" title="If you have NO discount code leave blank. <br>If you already paid for this class with a discount website or got a discount from Treks and Tracks, please enter discount code here. <br><br>Question? Call us at 650-557-4893 Mon-Fri, 9-5 PST or email info@treksandtracks.com" href="#"> Help</a> </label>
						</li>
						<li>
								<input type="text" 
						class="text" 
...
"xajax_verify_promo_code(xajax.getFormValues('booking<?= $j ?><?= $i ?>'),<?= $j ?><?= $i ?>,<?=$event['event_event_id'] ?>,<?=$event['exp_discount_price'] ?>);return false;" size="10" id="dspl_promo_code<?= $j ?><?= $i ?>" name="update" value="CONTINUE" class="submit" />
						</div>
						</form>
				</ul>
				</p>
