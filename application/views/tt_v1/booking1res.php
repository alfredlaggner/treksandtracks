...
//			animated: "easeslide",
   ...
   function stopEnterKey(evt) {
    ...
    document.onkeypress = stopEnterKey;</script>
...
echo form_open('tt_v1/class_booking/'. $event['event_event_id'] . "/0" ,$attributes);?>
					<? $max_part = ($event['capacity'] - $event['attending']) ?>
						...
						<li><label for="promo_code"   >Enter Discount Code <a class="tooltip" title="If you have NO discount code leave field blank. <br>If you already paid for this class with a discount website or got a discount from Treks and Tracks, please enter your discount code now. <br><br>Question? Call us at 650-557-4893 Mon-Fri, 9-5 PST or email info@treksandtracks.com" href="#">Help</a> </label>
						</li>
						<li><input type="text" class="text" onKeyDown="displayResult<?= $j; ?><?= $i; ?>()"   onKeyUp=
"xajax_verify_promo_code(xajax.getFormValues('booking<?= $j ?><?= $i ?>'),<?= $j ?><?= $i ?>,<?=$event['event_event_id'] ?>);return false;" size="10" id="promo_code<?= $j ?><?= $i ?>"  name="promo_code" value=""></li>
			<div id="customer_data">				
