...
<tbody>		
	...
	<? endforeach; endif ?>
		...
		<tr>
		<td>Total Price</td>
		<td></td><td><bold><?= '$' .number_format($total_price,2) ; ?></bold></td></tr>

</table>
<? if ($row->promo_code) : ?>
		<? if ($row->exp_discount_price == 0) :?>
	<h4 style="color: red" >If a scheduling conflict arises and you need to reschedule, we are happy to reactivate your voucher. If a reschedule is requested within 10 days prior to your class, a fee will apply. (Rock climbing $25, backpacking $45) </h4>	
		<? else : ?>
			<h4 style="color: red; padding-top:15px;">Your voucher is expired. Nevertheless you can still attend the class for a small fee.</h4>
        <? endif ?>
 <? endif ?>
...
<td>
					<label>First</label>
			<input type='text' class="text validate[required,x_first_name] " id='x_first_name' name='x_first_name' size="20" value='<?= $x_first_name; ?>' />
</td>			
<td>			
			<label>Last</label>
			<input type='text' class="text validate[required,x_last_name] " id='x_last_name' name='x_last_name' size="20" value='<?= $x_last_name; ?>' />
</td>			
</tr>
...
<td>
			<label>Address</label>
			<input type='text' class="text  validate[required,x_address] " id='x_address' name='x_address' size="20" value='<?= $x_address; ?>' />
</td>
</tr>
<tr>
<td>
			<label>City</label>
			<input type='text' class="text  validate[required,x_city] " id='x_city' name='x_city' size="20" value='<?= $x_city; ?>' />
</td>
<td>			
			
			<label>Zip</label>
			<input type='text' class="text  validate[required,x_zip] " id='x_zip' name='x_zip' size="4" value='<?= $x_zip; ?>' />
</td>
</tr>
...
<td>			
			...
			<label>State</label>
			<input type='text' class="text  validate[required,x_state] " id='x_state' name='x_state' size="2" value='<?= $x_state; ?>' />
</td>			


<td>
			<label>Country</label>
			<input type='text' class="text" id='x_country' name='x_country' value='<?= $x_country; ?>' />
</td>
</tr>
</table>
			</fieldset>
...
<td>			
					...
					<input id="creditcard" type="text" class="validate[required,creditCard] text " size="16" name="x_card_num" value=""><!--6011000000000012-->
					</input>
</td>
<td>				
					<label>Exp</label>
					<input id="exp" type="text"  class="text validate[required]" size="4" name="x_exp_date" value="">
					</input>
</td>
<td>					
					<label>CCV</label>
					<input  id="ccv" type="text" class="text validate[required]" size="4" name="x_card_code" value=""><!--782-->
					</input>
</td>
</tr>	
</fieldset>	
...
<? if ($row->promo_code) : ?>
		<? if ($row->exp_discount_price > 0) :?>
			<h4 style="color: red">Your voucher is expired. Nevertheless you can still attend the class for a very small fee.</h4>
        <? endif ?>
 <? endif ?>
...
<div id="sitelock_shield_logo" class="fixed_btm" style="bottom:0;position:fixed;_position:absolute;right:0;"><a href="https://www.sitelock.com/verify.php?site=www.treksandtracks.com" onclick="window.open('https://www.sitelock.com/verify.php?site=www.treksandtracks.com','SiteLock','width=600,height=600,left=160,top=170');return false;" ><img alt="SiteLock" title="SiteLock" src="//shield.sitelock.com/shield/www.treksandtracks.com"></a></div>
</body>
</html>