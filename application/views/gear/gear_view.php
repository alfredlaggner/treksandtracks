...
$().ready(function() {
	$("#gear").validate(
	{
		...
		messages: {
			...
			name: 	{
				required: "Please enter an gear name",
...
<div id="sub-nav">
	<div class="page-title">
		<h1><?= $title ?></h1>
		<span><a href="#" title="Home">Home</a> > <a href="#" title="Dashboard">Dashboard</a> > <?= anchor('gear', 'gear Overview'); ?> > <?= $title_action ?></span> </div>
		...
		<div id="page-content-wrapper">
			...
			<div id="tabs">
					<div class="content-box">
						<div id="inputform">
							<ul border="0">
								<? $attributes = array('id' => 'gear'); ?>
								<?= form_open('gear/update/'.$row->gear_id,$attributes );?>
								<li>
									<input type="hidden"  name="gear_id" id="gear_id"  value='<?= $row->gear_id; ?>'/>
								</li><a href="../../../images">Treks and Tracks</a>
<!--							<li>
								<label>Account</label>
...
-->								<li>
							...
							<li>
								<label>Group</label>
								<select type="text" name="gear_group_id" id="gear_group_id" class="text"  value='<?= $row->gear_group_id; ?>'  />
								
								<? if(isset($gear_groups)) : foreach($gear_groups as $gear_group) : ?>
								<? if ($row->gear_group_id == $gear_group->gear_group_id) : ?>
								<option selected value="<?=  $gear_group->gear_group_id; ?>"><?=  $gear_group->name; ?></option>
								<? else : ?>
								<option          value="<?=  $gear_group->gear_group_id; ?>"><?=  $gear_group->name; ?></option>
								<? endif; ?>
								<? endforeach; ?>
								<? endif; ?>
								</select>
