		...
		<div id="page-content">
				<div id="page-content-wrapper">
						<div class="inner-page-title">
								...
								<span><?= $top_note ?></span>
								<p style="float:right";>
										<?= anchor('gear/gear_create', 'Create New gear')?>
								</p>
						</div>
						<div class="hastable">
										...
										<table id="sort-table">
												...
												<tbody>
														...
														<tr>
																...
																<td><?= $row->name; ?></td>
																<td><?= $row->code; ?></td>
																<td><?= $this->gear_group_model->get_gear_group_name($row->gear_group_id); ?></td>
																<td><?= $row->order; ?></td>
																<td><? if($row->is_featured) echo "Yes"; else echo "No"; ?></td>
																<td><? if($row->is_active)   echo "Yes"; else echo "No"; ?></td>
																<td><a class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Edit gear" href="<?= site_url().'gear/gear_view/' ?><?= $row->gear_id; ?>"> <span class="ui-icon ui-icon-wrench"></span> </a> 
																<a class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Edit Pictures" href="<?= site_url().'gear_pictures/index/0/' ?><?= $row->gear_id . '/' .$row->code  ?>"> <span class="ui-icon ui-icon-image"></span> </a> 

																<a class="btn_no_text btn ui-state-default ui-corner-all tooltip" title="Related Gears" href="<?= site_url().'gear_related/index/' ?><?= $row->gear_id  ?>"> <span class="ui-icon ui-icon-link"></span> </a> 
																
																
																<a class="btn_no_text btn ui-state-default ui-corner-all tooltip confirmClick" title="Delete gear" href="<?= site_url().'gear/delete/' ?><?= $row->gear_id; ?>"> <span class="ui-icon ui-icon-trash"></span> </a></td>
														</tr>
														<? endforeach; endif; ?>
												</tbody>
										</table>
