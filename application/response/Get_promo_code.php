...
class Get_promo_code extends CI_Controller {
	
	function get_promo_code($promo_code,$count) {
		if (!$promo_code) return FALSE;
		
		$row= $this->discount_model->get_promo_code($promo_code);
		$ajax = ajax();
		
		if ($row)
			{
			$type = $row->amount_type == "F" ? "fixed" : "%";
			$answer = " -  Yeah -  code exists! :-)" ;
			$answer .= "<br>For :"  . $row->activity_name;
			$answer.= "<br> Value: $" . $row->amount . " " .$type ;
			$answer .= "<br>Valid Until :"  . $row->exp_date;
			
			}
		else
		   $answer = $promo_code . " - Ooops - Not found! :-(";
		   
		  $ajax->update("ajax_message".$count, $answer); 
	}
